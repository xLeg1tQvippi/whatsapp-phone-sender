...
class MainWindowSender(QWidget):
    ...
    def pack_number(self, text, language):
        try:
            pack = []; pack.append(text); pack.append(language)
        except:
    ...
    def check_num(self, number):
        ...
        try:
            ...
            num = self.check_first_letter(number) #Возвращает 0 индексом - True/False; 1 индекс - номер телефона стадия 1.
            if num[0] == True:
                print('FLS:', num[0])
                ...
                print(f'FLS: {num[0]}')
                raise num[2]
        except (CheckFirstLetterError, SpecialSignCleanerError, InOldListNumber, PlusError, IncorrectNumber, IncorrectNumberType, InActiveListNumber) as error:
